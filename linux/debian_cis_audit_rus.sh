...
#
# Комплексный технический аудит безопасности Debian/Ubuntu
# систем (DEB/Linux). Ориентирован на технические контроли ISO/IEC 27001
...
#   - /var/log/debian_cis_audit_report.html  (современный интерактивный веб-отчёт)
...
log_raw "${C_BOLD}Технический аудит безопасности (CIS / ISO 27001) - Debian/Ubuntu Linux${C_RESET}"
...
if [[ ${DEBIAN_FAMILY} -ne 1 ]]; then
    warn "ОС не определена как Debian/Ubuntu-compatible (${OS_INFO}); Debian-специфичные проверки могут быть неприменимы" \
         "Запускайте этот аудит на Debian/Ubuntu и проверьте /etc/os-release"
else
    pass "Обнаружена Debian/Ubuntu-compatible система: ${OS_INFO}"
fi
...
if [[ -f /etc/logrotate.conf ]]; then
    ...
    fail "Файл /etc/logrotate.conf отсутствует" "apt install -y logrotate"
fi
...
if have_cmd ss; then
    ...
    if [[ "${WILDCARD_LISTEN}" -gt 0 ]]; then
        warn "Обнаружено ${WILDCARD_LISTEN} TCP-портов, слушающих на всех интерфейсах (0.0.0.0/::)" \
             "Ограничить bind-адрес прикладных сервисов конкретным интерфейсом; закрыть неиспользуемые порты в UFW/iptables"
    fi
...
if have_cmd ufw && ufw status | grep -q "Status: active"; then
    ...
    info "UFW Политики: ${UFW_DEFAULT}"
    if echo "${UFW_DEFAULT}" | grep -qE "deny \(incoming\)|reject \(incoming\)"; then
        pass "Политика по умолчанию для входящего трафика: deny/reject (implicit deny, безопасно)"
    else
        fail "Политика по умолчанию для входящего трафика разрешает соединение" \
             "ufw default deny incoming && ufw reload"
    fi
elif have_cmd nft && nft list ruleset | grep -q 'table'; then
    FW_CONFIGURED=1
    pass "nftables активен и содержит правила"
    ...
    fail "Не обнаружено активного межсетевого экрана (UFW, nftables, iptables)" "apt install -y ufw && ufw enable"
fi
[[ "${FW_CONFIGURED}" -eq 0 ]] && fail "Активный межсетевой экран не обнаружен" "ufw enable"
...
# На Debian/Ubuntu /etc/shadow и /etc/gshadow обычно имеют группу shadow (0640)
...
if have_cmd getfacl; then
    ...
    warn "Утилита getfacl не найдена (пакет acl)" "apt install -y acl"
fi
...
if [[ -f "${SSHD_CONFIG}" ]]; then
    ...
    fail "Файл ${SSHD_CONFIG} не найден" "apt install -y openssh-server"
fi

# ============================================================================
# 5. СЛУЖБЫ И УСТАНОВЛЕННЫЕ ПАКЕТЫ (DEB / APT)
# ============================================================================

log_section "5. СЛУЖБЫ, ОБНОВЛЕНИЯ И ЦЕЛОСТНОСТЬ DEB ПАКЕТОВ"
        ...
        if svc_loaded "${svc}"; then
            ...
            if [[ "${state}" == "active" || "${enabled}" == "enabled" ]]; then
                fail "Небезопасная служба активна/включена: ${svc} (active=${state}, enabled=${enabled})" \
                     "systemctl disable --now ${svc} && apt purge -y <пакет-службы>"
                FOUND_INSECURE=1
            else
...
if have_cmd debsums; then
    info "Запуск проверки целостности пакетов (debsums -c)..."
    ...
    if [[ -z "${CRITICAL_BIN_CHANGES}" ]]; then
        ...
        fail "Обнаружены изменения в системных бинарных файлах, затронуто объектов: ${CHANGE_COUNT}" \
             "apt-get --reinstall install <пакет>"
    ...
    warn "Утилита debsums не установлена. Проверка контрольных сумм бинарников пропущена." \
         "apt install -y debsums"
fi

if have_cmd apt; then
    info "Проверка доступных обновлений через apt..."
    ...
    warn "Утилита apt не найдена"
fi

# ============================================================================
# 6. ЗАГРУЗЧИК GRUB2, КОНСОЛЬ И APPARMOR / SELINUX (CIS 1.4 / 1.6)
# ============================================================================

log_section "6. ЗАГРУЗЧИК GRUB2, КОНСОЛЬ И APPARMOR/SELINUX"
...
if [[ -n "${GRUB_CFG}" ]]; then
    if grep -qE '^\s*password' "${GRUB_CFG}" || grep -qE '^\s*password_pbkdf2' "${GRUB_CFG}" || [[ -f /etc/grub.d/40_custom_user ]]; then
        pass "GRUB2 защищён паролем"
    else
        fail "Загрузчик GRUB2 НЕ защищён паролем (CIS 1.4.2)" \
             "grub-mkpasswd-pbkdf2 и задать пользователя/пароль в /etc/grub.d/40_custom"
    fi
...
# В Debian/Ubuntu основным LSM является AppArmor, но может быть установлен и SELinux
if have_cmd aa-status; then
    if aa-status --enabled 2>/dev/null; then
        pass "AppArmor активен и включен (CIS 1.6)"
    else
        fail "AppArmor ОТКЛЮЧЕН" "systemctl enable --now apparmor"
    fi
elif have_cmd getenforce; then
    SELINUX_STATE="$(getenforce)"
    if [[ "${SELINUX_STATE}" == "Enforcing" ]]; then
        pass "SELinux активен и находится в режиме Enforcing"
    else
        warn "SELinux состояние: ${SELINUX_STATE}" "setenforce 1"
    fi
else
    fail "Не найдены утилиты AppArmor (aa-status) или SELinux (getenforce)" \
...
<div class="container">
  ...
  <div class="header-card">
    <div class="header-title">
      <h1>Отчёт аудита безопасности Debian/Ubuntu (CIS / ISO 27001)</h1>
...
HTMLHEAD
  ...
  <footer>
    Сгенерировано автоматически <code>debian_cis_audit.sh</code> (v${SCRIPT_VERSION}) &copy; ${AUDITOR_NAME}
  </footer>
</div>
