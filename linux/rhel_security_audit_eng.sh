...
#
# Comprehensive technical security audit of Red Hat Enterprise Linux (RHEL)
# systems (RPM/Linux). Targeted at ISO/IEC 27001 technical controls
# (Annex A: A.8, A.12, A.13) and CIS Benchmarks for Linux methodology
# (Level 1 / Level 2).
#
# The script only READS system state (read-only audit).
#   [PASS] - control satisfied
#   [WARN] - recommendation deviation / Level 2 / requires attention
#   [FAIL] - critical non-compliance (Level 1)
#   [INFO] - informational line, does not affect overall evaluation
#
# Output:
#   - console (colorized)
#   - /var/log/rhel_cis_audit_report.log   (text report, cumulative)
#   - /var/log/rhel_cis_audit_report.html  (modern interactive web report)

set -u
set -o pipefail

# ============================================================================
# 0. GLOBAL PARAMETERS AND INITIALIZATION
# ============================================================================
...
CURRENT_SECTION="1. INITIALIZATION AND OS DETAILS"
...
# ============================================================================
# 1. ROOT CHECK AND OS DETAILS
# ============================================================================

log_section "1. INITIALIZATION AND OS DETAILS"

if [[ "$(id -u)" -ne 0 ]]; then
    echo -e "${C_RED}${C_BOLD}[FAIL] Script must be run as root (UID 0).${C_RESET}" >&2
    echo "Current user: $(id -un) (UID $(id -u))" >&2
    echo "Re-run command: sudo bash $0" >&2
    exit 1
fi

log_raw "${C_BOLD}Technical Security Audit (CIS / ISO 27001) - Red Hat Enterprise Linux${C_RESET}"
...
if [[ ${RHEL_FAMILY} -ne 1 ]]; then
    warn "OS is not identified as RHEL/RHEL-compatible (${OS_INFO}); RHEL-specific checks may not apply" \
         "Run this audit on RHEL 8/9/10 or a compatible RPM system and check /etc/os-release"
else
    pass "RHEL/RHEL-compatible system detected: ${OS_INFO}"
fi

# ============================================================================
# 2. LOGGING SUBSYSTEM AUDIT (auditd / rsyslog / journald / logrotate)
# ============================================================================
...
if have_cmd systemctl; then
    if svc_loaded auditd.service; then
        ...
        if [[ "${AUDITD_ENABLED}" == "enabled" ]]; then
            pass "auditd is enabled at boot (is-enabled: enabled)"
        else
            fail "auditd is NOT enabled at boot (is-enabled: ${AUDITD_ENABLED:-unknown})" \
...
if [[ "${AUDITD_ACTIVE}" == "active" ]] && have_cmd auditctl; then
    ...
    if [[ "${RULES_COUNT}" -eq 0 ]] || echo "${AUDIT_RULES}" | grep -qi "no rules"; then
        ...
        pass "Audit rules detected: ${RULES_COUNT}"
        if echo "${AUDIT_RULES}" | grep -q "/etc/passwd"; then
            pass "Audit rule for /etc/passwd modifications is present"
        else
            fail "Missing audit rule for /etc/passwd modifications" \
        ...
        if echo "${AUDIT_RULES}" | grep -q "/etc/shadow"; then
            pass "Audit rule for /etc/shadow modifications is present"
        else
            fail "Missing audit rule for /etc/shadow modifications" \
        ...
        if echo "${AUDIT_RULES}" | grep -qE "/etc/sudoers"; then
            pass "Audit rule for /etc/sudoers modifications is present"
        else
            fail "Missing audit rule for /etc/sudoers modifications" \
                 "echo '-w /etc/sudoers -p wa -k scope' >> /etc/audit/rules.d/scope.rules && augenrules --load"
        fi
        if echo "${AUDIT_RULES}" | grep -q "execve"; then
            pass "Audit rule for execve system calls is present"
        else
        ...
        if echo "${AUDIT_RULES}" | grep -qE "/etc/hosts|/etc/sysconfig/network|network"; then
            pass "Audit rule for network configuration changes is present"
        else
    ...
    warn "auditctl utility not found - audit rules content not verified"
fi
...
if have_cmd systemctl; then
    for svc in rsyslog systemd-journald; do
        if svc_loaded "${svc}.service"; then
            ...
            info "Unit ${svc}.service is missing from the system"
        fi
    done
fi
...
if [[ -f /etc/systemd/journald.conf ]]; then
    if grep -qE '^\s*Storage\s*=\s*persistent' /etc/systemd/journald.conf; then
        ...
        warn "journald is not configured with Storage=persistent - logs will not survive reboot" \
...
if [[ -f /etc/logrotate.conf ]]; then
    ...
    if [[ -n "${LR_ROTATE:-}" ]]; then
        ...
        info "'rotate' parameter is not explicitly defined in logrotate.conf"
    fi
    if grep -qE '^\s*compress\s*$' /etc/logrotate.conf; then
        pass "Log compression for rotated logs is enabled (compress)"
    else
    ...
    if [[ -d /etc/logrotate.d ]]; then
        ...
        info "Drop-in configurations found in /etc/logrotate.d: ${LR_DROPINS}"
    fi
else
    fail "File /etc/logrotate.conf is missing" "dnf install logrotate"
fi

# ============================================================================
# 3. NETWORK, PORTS AND FIREWALL SCANNING
# ============================================================================

log_section "3. NETWORK SECURITY AND FIREWALL"

if have_cmd ss; then
    info "Gathering open ports data via 'ss -tulpn'"
    PORT_DATA="$(ss -tulpn )"
    ...
    if [[ "${WILDCARD_LISTEN}" -gt 0 ]]; then
        warn "Detected ${WILDCARD_LISTEN} TCP ports listening on all interfaces (0.0.0.0/::)" \
             "Restrict application service bind addresses to specific interfaces; close unused ports in firewalld"
    fi
elif have_cmd netstat; then
    info "ss not found, using netstat -tulpn"
    netstat -tulpn  | tail -n +3 | while IFS= read -r line; do
        log_raw "         ${line}"
    done
else
    warn "Neither ss nor netstat were found"
fi
...
if have_cmd firewall-cmd && systemctl is-active firewalld >/dev/null 2>&1; then
    ...
    info "Default zone: ${FW_ZONE:-unknown}, target: ${FW_TARGET:-unknown}"
    if [[ "${FW_TARGET}" == "DROP" || "${FW_TARGET}" == "REJECT" || "${FW_TARGET}" == "default" ]]; then
        pass "Default policy for zone '${FW_ZONE}': ${FW_TARGET} (implicit deny, secure)"
    elif [[ "${FW_TARGET}" == "ACCEPT" ]]; then
        fail "Default policy for zone '${FW_ZONE}': ACCEPT - all incoming traffic is allowed" \
             "firewall-cmd --permanent --zone=${FW_ZONE} --set-target=default && firewall-cmd --reload"
    else
        warn "Default policy for zone '${FW_ZONE}': ${FW_TARGET:-unknown} - check manually"
    fi
    ...
    info "Allowed services in zone '${FW_ZONE}': ${FW_OPEN_SERVICES:-none}"
    info "Allowed ports in zone '${FW_ZONE}': ${FW_OPEN_PORTS:-none}"
    ...
    fail "Neither firewalld nor iptables detected" "dnf install firewalld && systemctl enable --now firewalld"
fi
[[ "${FW_CONFIGURED}" -eq 0 ]] && fail "No active firewall detected" "systemctl enable --now firewalld"
...
# ============================================================================
# 4. USERS, ACCESS PERMISSIONS, ACL AND SSH
# ============================================================================

log_section "4. USER ACCOUNTS, ACCESS PERMISSIONS AND SSH SERVICE"

UID0_USERS="$(awk -F: '($3 == 0) {print $1}' /etc/passwd | grep -v '^root$')"
if [[ -z "${UID0_USERS}" ]]; then
    pass "Only root has UID 0"
else
    fail "Additional accounts with UID 0: ${UID0_USERS//$'\n'/, }" \
...
if [[ -r /etc/shadow ]]; then
    ...
    if [[ -z "${EMPTY_PW_USERS}" ]]; then
        ...
        fail "Accounts with EMPTY password: ${EMPTY_PW_USERS//$'\n'/, }" \
             "passwd -l <user> or set password: passwd <user>"
    fi
    LOCKED_MISMATCH="$(awk -F: '($2 !~ /^!|^\*/ && $2 != "") {print $1}' /etc/shadow | wc -l)"
    info "Accounts with a set password hash: ${LOCKED_MISMATCH}"
else
...
if have_cmd getfacl; then
    for dir in /var/log /etc /home; do
        ...
        if [[ "${ACL_ENTRIES}" -gt 0 ]]; then
            info "${dir}: objects with non-standard ACLs: ${ACL_ENTRIES} (manual review required)"
        else
            pass "${dir}: no non-standard ACLs detected"
        fi
    done
else
    warn "getfacl utility not found (acl package)" "dnf install acl"
fi
...
if [[ -f "${SSHD_CONFIG}" ]]; then
    ...
    if [[ -z "${PROTO}" || "${PROTO}" == "2" ]]; then
        ...
        fail "SSH Protocol = '${PROTO}' - deprecated protocol configuration detected" \
    ...
    if [[ -n "${CIPHERS}" ]]; then
        if echo "${CIPHERS}" | grep -qiE 'arcfour|3des|blowfish|cbc'; then
            fail "Weak SSH ciphers allowed: ${CIPHERS}" \
                 "Set Ciphers chacha20-poly1305@openssh.com,aes256-gcm@openssh.com,aes128-gcm@openssh.com"
        else
            pass "Ciphers explicitly set, no weak algorithms detected: ${CIPHERS}"
        fi
    else
        info "Ciphers directive is not explicitly defined - using OpenSSH default set"
    fi

    SSHD_PERM="$(stat -c '%a' "${SSHD_CONFIG}" )"
    if [[ $((10#${SSHD_PERM:-0})) -eq 600 || $((10#${SSHD_PERM:-0})) -eq 644 ]]; then
        pass "${SSHD_CONFIG}: access permissions ${SSHD_PERM}"
    else
        warn "${SSHD_CONFIG}: access permissions ${SSHD_PERM} (recommended 0600)" \
             "chmod 600 ${SSHD_CONFIG}"
    fi
else
    fail "File ${SSHD_CONFIG} not found" "dnf install openssh-server"
fi

# ============================================================================
# 5. SERVICES AND INSTALLED PACKAGES (RPM)
# ============================================================================

log_section "5. SERVICES, UPDATES AND RPM INTEGRITY"
        ...
        if svc_loaded "${svc}"; then
            ...
            if [[ "${state}" == "active" || "${enabled}" == "enabled" ]]; then
                fail "Insecure service active/enabled: ${svc} (active=${state}, enabled=${enabled})" \
                     "systemctl disable --now ${svc} && dnf remove <service-package>"
                FOUND_INSECURE=1
            else
                info "Insecure service ${svc} is installed, but inactive and disabled at boot"
            fi
        fi
    done
...
if have_cmd rpm; then
    info "Running package integrity verification (rpm -Va)..."
    ...
    if [[ -z "${CRITICAL_BIN_CHANGES}" ]]; then
        pass "No modified binary files detected in /bin,/sbin,/usr/bin,/usr/sbin"
    else
        ...
        fail "Modifications detected in system binary files, affected objects: ${CHANGE_COUNT}" \
             "rpm -Vf <file>; reinstall package if necessary: rpm -Uvh --replacepkgs <package>"
    ...
    [[ "${MISSING_FILES}" -gt 0 ]] && warn "rpm -Va: missing package files: ${MISSING_FILES}"
else
    warn "rpm utility not found"
fi

if have_cmd dnf; then
    info "Checking available updates via dnf..."
    ...
    if [[ ${UPDATE_RC} -eq 100 ]]; then
        ...
        warn "Package updates available: ~${UPDATE_COUNT}" \
             "dnf update --security (test prior to production deployment)"
    elif [[ ${UPDATE_RC} -eq 0 ]]; then
        pass "System is fully up-to-date"
    else
        warn "Failed to check updates via dnf (rc=${UPDATE_RC})"
    fi
elif have_cmd yum; then
    info "Checking available updates via yum..."
    UPDATE_OUTPUT="$(yum -q check-update )"; UPDATE_RC=$?
    if [[ ${UPDATE_RC} -eq 100 ]]; then
        UPDATE_COUNT="$(echo "${UPDATE_OUTPUT}" | grep -cE '^\S+\.\S+\s+\S+\s+\S+$')"
        warn "Package updates available: ~${UPDATE_COUNT}" "yum update --security"
    elif [[ ${UPDATE_RC} -eq 0 ]]; then
        pass "System is fully up-to-date"
    else
        warn "Failed to check updates via yum (rc=${UPDATE_RC})"
    fi
else
    warn "Neither dnf nor yum were found"
fi

# ============================================================================
# 6. GRUB2 BOOTLOADER, CONSOLE AND SELINUX (CIS 1.4 / 1.6)
# ============================================================================

log_section "6. GRUB2 BOOTLOADER, CONSOLE AND SELINUX"
...
if [[ -n "${GRUB_CFG}" ]]; then
    if grep -qE '^\s*password' "${GRUB_CFG}" || grep -qE '^\s*password_pbkdf2' "${GRUB_CFG}" || [[ -f /boot/grub2/user.cfg ]]; then
        pass "GRUB2 is password protected (user.cfg / password_pbkdf2)"
    else
    ...
    if [[ $((10#${GRUB_PERM:-0})) -eq 600 || $((10#${GRUB_PERM:-0})) -eq 400 ]]; then
        pass "${GRUB_CFG}: access permissions ${GRUB_PERM}"
    else
        fail "${GRUB_CFG}: access permissions ${GRUB_PERM} (expected 0600/0400)" \
             "chmod 600 ${GRUB_CFG}"
    fi
else
...
if have_cmd getenforce; then
    ...
    if [[ "${SELINUX_STATE}" == "Enforcing" ]]; then
        pass "SELinux is active and in Enforcing mode (CIS 1.6.1.2)"
    elif [[ "${SELINUX_STATE}" == "Permissive" ]]; then
        warn "SELinux is in Permissive mode (logs but does not block)" \
             "setenforce 1 && sed -i 's/^SELINUX=.*/SELINUX=enforcing/' /etc/selinux/config"
    else
        fail "SELinux is DISABLED (state: ${SELINUX_STATE})" \
             "sed -i 's/^SELINUX=.*/SELINUX=enforcing/' /etc/selinux/config"
    fi
else
    fail "getenforce utility not found (SELinux is not installed)" "dnf install selinux-policy-targeted libselinux-utils"
fi

if [[ -f /etc/sysconfig/init ]]; then
    if grep -qE '^\s*SINGLE=/sbin/sulogin' /etc/sysconfig/init; then
        pass "Single-user mode requires authentication (SINGLE=/sbin/sulogin)"
    else
        warn "Single-user mode is not enforced to sulogin in /etc/sysconfig/init"
    fi
fi
...
if svc_loaded "debug-shell.service"; then
    ...
    if [[ "${DEBUG_SHELL_ACTIVE}" == "active" ]]; then
        ...
        pass "debug-shell.service is inactive"
    fi
fi

# ============================================================================
# 7. KERNEL, MEMORY MANAGERS AND MEMORY SECURITY
# ============================================================================

log_section "7. KERNEL SECURITY AND MEMORY PROTECTION"

ASLR_VAL="$(sysctl -n kernel.randomize_va_space  || echo 0)"
if [[ "${ASLR_VAL}" -eq 2 ]]; then
    pass "ASLR fully enabled (kernel.randomize_va_space = 2)"
elif [[ "${ASLR_VAL}" -eq 1 ]]; then
    warn "ASLR partially enabled (kernel.randomize_va_space = 1)" \
         "sysctl -w kernel.randomize_va_space=2 && echo 'kernel.randomize_va_space = 2' >> /etc/sysctl.d/99-cis-hardening.conf"
else
    fail "ASLR DISABLED (kernel.randomize_va_space = ${ASLR_VAL})" \
         "sysctl -w kernel.randomize_va_space=2 && echo 'kernel.randomize_va_space = 2' >> /etc/sysctl.d/99-cis-hardening.conf"
fi

PTRACE_VAL="$(sysctl -n kernel.yama.ptrace_scope  || echo 0)"
if [[ "${PTRACE_VAL}" -ge 1 ]]; then
    pass "ptrace protection restricted (kernel.yama.ptrace_scope = ${PTRACE_VAL})"
else
    warn "ptrace not restricted (kernel.yama.ptrace_scope = 0)" \
...
if [[ "${DUMPABLE}" -eq 0 ]]; then
    pass "Core dumps for SUID processes disabled (fs.suid_dumpable = 0)"
else
...
if [[ -f /etc/security/limits.conf ]]; then
    if grep -qE '^\*\s+hard\s+core\s+0' /etc/security/limits.conf /etc/security/limits.d/*.conf 2>/dev/null; then
        ...
        warn "'hard core 0' restriction not found in /etc/security/limits.conf" \
...
if [[ "${KEXEC_VAL}" -eq 1 ]]; then
    pass "Loading new kernel via kexec_load disabled (kernel.kexec_load_disabled = 1)"
else
...
if [[ $? -eq 0 ]]; then
    log_raw ""
    log_raw "Text report saved/appended: ${REPORT_FILE}"
else
    log_raw ""
...
# ============================================================================
# 10. GENERATING UPDATED HTML REPORT (UI/UX 2026)
# ============================================================================
...
<div class="container">
  ...
  <div class="header-card">
    <div class="header-title">
      <h1>RHEL Security Audit Report (CIS / ISO 27001)</h1>
      <p>Automated Technical Compliance Verification System</p>
    </div>
  ...
  <div class="dashboard-grid">
    <div class="score-card">
      ...
      <div style="font-size:12px; color:var(--text-muted); margin-top:2px;">Based on CIS Benchmarks compliance</div>
    </div>
    ...
    <div class="kpi-cards">
      ...
      <div class="kpi-card fail">
        <span class="kpi-title">Failed (FAIL)</span>
        <span class="kpi-value">${COUNT_FAIL}</span>
      </div>
      <div class="kpi-card info">
        <span class="kpi-title">Info (INFO)</span>
  ...
  <div class="filter-bar">
    ...
    <input type="text" id="searchInput" class="search-input" placeholder="Search by checks or sections..." onkeyup="searchTable()">
  </div>
    ...
    <table id="auditTable">
      <thead>
        <tr>
          ...
          <th>Check Result / Step-by-Step Recommendation</th>
        </tr>
      </thead>
      <tbody>
HTMLHEAD
  ...
  <footer>
    Automatically generated by <code>rhel_cis_audit.sh</code> (v${SCRIPT_VERSION}) &copy; ${AUDITOR_NAME}
  </footer>
</div>
...
HTMLFOOT
    ...
    if [[ $? -eq 0 ]]; then
        ...
        log_raw "${C_YELLOW}[WARN] Failed to write HTML report ${HTML_REPORT_FILE}${C_RESET}"
    fi
}
